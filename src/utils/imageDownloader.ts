...
export async function downloadImage({
  ...
  const processDownload = () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (cellData && !cellData.isExternal) {
          ...
          if (showCellNumbers) {
            ...
            const keyFont = Math.max(8, Math.floor(downloadCellSize * 0.42));
            ctx.font = `bold ${keyFont}px sans-serif`;
    ...
    if (includeStats && colorCounts) {
      ...
      ctx.font = `bold ${Math.max(16, statsFontSize + 2)}px sans-serif`;
      ctx.textAlign = 'left';
      ctx.textBaseline = 'alphabetic';
      ctx.fillText('色块统计', statsPadding, statsY);

      const colorCountLabel = `共 ${colorKeys.length} 色`;
      const totalLabel = `总色块 ${totalBeadCount} 颗`;
      ctx.font = `600 ${Math.max(13, statsFontSize)}px sans-serif`;
      ctx.fillStyle = '#4B5563';
      ctx.textAlign = 'right';
      ctx.fillText(`${colorCountLabel}  ·  ${totalLabel}`, downloadWidth - statsPadding, statsY);

      // 绘制分隔线
      ctx.strokeStyle = '#DDDDDD';
      ctx.beginPath();
      ctx.moveTo(statsPadding, statsY + 12);
      ctx.lineTo(downloadWidth - statsPadding, statsY + 12);
      ctx.stroke();
      
      const titleHeight = 36; // 标题和分隔线的总高度
      // 根据色块大小动态调整行高
      ...
      // 按显示色号排序后绘制
      const sortedKeys = [...colorKeys].sort((a, b) =>
        sortColorKeys(
      ...
      // 计算实际需要的行数
      const numRows = Math.ceil(sortedKeys.length / renderNumColumns);
      
      // 绘制总量
      const totalY = statsY + titleHeight + (numRows * statsRowHeight) + 14;
      ctx.font = `bold ${Math.max(14, statsFontSize + 1)}px sans-serif`;
      ctx.textAlign = 'right';
      ctx.fillStyle = '#111827';
      ctx.fillText(`总计色块：${totalBeadCount} 颗（${sortedKeys.length} 色）`, downloadWidth - statsPadding, totalY);
    ...
    try {
      ...
      resolve({ dataUrl: dataURL, filename });
    } catch (e) {
